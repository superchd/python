...
def KeypointProjection(xy_points, h):
    ...
    xy_size = xy_points.shape
    h_size  = h.shape

    # xy_points_out = np.ndarray(xy_points_out)
    xy_h = np.array([])

    #  x, y 좌표에 마지막 원소를 1 추가해서 homography 기본 세팅을 해주기
    for i in range(xy_size[0]):
        ...
        #  행렬곱 실행
        #  xy_h = xy_points[i] * h
        #  #  마지막 원소를 1로 만들어주면서, x, y 좌표를 나누어준다
        #  xy_h[0] = xy_h[0] / xy_h[2]
        #  xy_h[1] = xy_h[1] / xy_h[2]
        #  print(xy_points[i])
        #  np.append(xy_points_out, xy_h)
        #
    print(xy_points_out)
    #  END
